    ...
    ?>
        <div class="principal">
            <div id="container">
                ...
                <form action="MODIFICAR_PRODUCTO2.php?user=<?=$user?>&producto=<?=$producto?>" method="post" id="contact_form" enctype="multipart/form-data" name="form" >
                  <div class="nombre">
                    <label for="name"></label>
                    <input type="text" placeholder="NOMBRE" name="nombre" value="<?=mysqli_real_escape_string($conn, $row['nom_producto'])?>" pattern="^[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+(\s*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]*)*[a-zA-ZÀ-ÿ\u00f1\u00d1\u00E0-\u00FC]+$" id="name_input" required>
                  </div>
                  <div class="precio">
                    <label for="precio"></label>
                    <input type="number" placeholder="PRECIO" name="precio" value="<?=mysqli_real_escape_string($conn, $row['precio_producto'])?>" min="1" max="999999" required>
                  </div>
                  <div class="stock">
                    <label for="stock"></label>
                    <input type="number" placeholder="STOCK" name="stock" value="<?=mysqli_real_escape_string($conn, $row['num_productos'])?>" min="1" max="999" required>
                  </div>
                  <div class="especificacion">
                    <label for="especificacion"></label>
                    <input type="text" placeholder="ESPECIFICACIONES" name="especificacion" value="<?=mysqli_real_escape_string($conn, $row['espns_producto'])?>" id="email_input" required>
                  </div> 
                  <div class="descripción">
                    <label for="descripción"></label>
                    <input type="text" placeholder="DESCRIPCIÓN" name="descripcion" value="<?=mysqli_real_escape_string($conn, $row['descrip_producto'])?>" id="email_input" required>
                  </div>
                  <div class="foto1">
                ...
                <option value="Sistemas digitales">Sistemas digitales</option>
                    ...
                    <input type="submit" value="MODIFICAR" name="modificar" id="form_button" />
